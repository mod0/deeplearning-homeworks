...
def sgd(shuffle_order_training, train_data, shuffle_order_testing, test_data, w, dw, z, dz, a, y, eta, maxiter):
    ...
    for epoch in xrange(maxiter):
        ...
        for i in shuffle_order_training:
            ...
            forward_prop(i, train_data, w, z, a, y)
            backward_prop(i, train_data, w, dw, z, dz, a, y)
        ...
        for i in shuffle_order_testing:
            ...
            forward_prop(i, test_data, w, z, a, y)
            # Compute testing loss
            total_testing_loss += distance_lossfn(y, label_to_ytrue(len(y), test_data[i, -1]))

        print "Total training loss in this epoch: ", total_training_loss
        print "Total testing loss in this epoch: ", total_testing_loss
        ...
        training_accuracy.append(get_accuracy(shuffle_order_training, train_data, w, z, a, y))
        
        # Find the accuracy using current weights on test data
        testing_accuracy.append(get_accuracy(shuffle_order_testing, test_data, w, z, a, y))

        # Stop once testing loss begins to increase again
        if len(total_testing_loss_fn_value) == 1 or (total_testing_loss < total_testing_loss_fn_value[-2]):
            # Save previous weights
            previous_weights = copy.deepcopy(w)
        else:
            break
    ...
    model['testing_accuracy_with_best_test_loss']          = testing_accuracy[-2]
...
def get_accuracy(shuffle_order, data, w, z, a, y):
    pred_label = []
    true_label = data[:, -1]
    # first get training accuracy
    for i in shuffle_order:
        forward_prop(i, data, w, z, a, y)
        pred_label.append(y_to_label(y))

    return (1.0 * sum(pred_label == true_label))/len(true_label)

def get_precision_recall_fscore(shuffle_order, data, w, z, a, y):
    pred_label = []
    true_label = data[:, -1]
    # first get training accuracy
    for i in shuffle_order:
    ...
    return skm.precision_recall_fscore_support(true_label, pred_label)
    
def forward_prop(example_id, data, w, z, a, y):
...
def backward_prop(example_id, data, w, dw, z, dz, a, y):
    # Right now only making it work for the given MLP
    x             =  data[example_id,:-1]
    ...
    dw[1][:,:]    =  np.outer(dz[1], a[0])
    dw[0][:,:]    =  np.outer(dz[0], x)
...
def initialize(eta = 0.01, maxiter = 30):
    train_data = loaddata('train_data.txt')
    print "Loaded training data of shape: ", train_data.shape, type(train_data)

    test_data = loaddata('test_data.txt')
    print "Loaded testing data of shape: ", test_data.shape, type(test_data)
    
    params                              = dict()
    params['train_data']                = train_data
    params['test_data']                 = test_data
    params['w']                         = [nr.rand(3, 2), nr.rand(3,3), nr.rand(2,3)]           
