...
def main(wordvecfile = "sentiment-data/word-vectors-refine.txt",
    ...
    # Setup GRU
    gru_cell      = tf.nn.rnn_cell.GRUCell(n_gru_cell)
    outputs, state = tf.nn.dynamic_rnn(gru_cell, indata, dtype=tf.float32)

    # weights for last softmax
    W = tf.Variable(tf.random_uniform([n_gru_cell, n_classes]))
    ...
    for epoch in xrange(maxiter):
        ...
        if ((epoch + 1) % 2 == 0):
            save_path = saver.save(sess, "models/pretrained_gru.ckpt", global_step=epoch)
